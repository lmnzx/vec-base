...
fn search_exhaustive<const N: usize>(
    ...
    let mut idx_sq_euc_dis: Vec<(usize, f32)> = enumerated_iter
        .map(|(idx, data)| (idx, data.sq_euc_dis(vector)))
        .collect();
...
fn analyze_recall_metrics<const N: usize>(
    ...
    for (i, true_neighbours) in exhaustive_results.iter().enumerate() {
        ...
        total_recall_pct += num_matches_with_brute_results / true_neighbours.len() as f32;
    }
...
fn main() {
    ...
    let exhaustive_results: Vec<std::collections::HashSet<i32>> = sample_idx
        .par_iter()
        .map(|&idx| search_exhaustive::<DIM>(&my_input_data, &my_input_data[idx as usize], TOP_K))
        .collect();
    let duration = start.elapsed();
    ...
    analyze_average_euclidean_metrics::<DIM>(&path, &my_input_data, &index_results, None);
