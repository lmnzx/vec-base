...
impl<const N: usize> Vector<N> {
    pub fn subtract_form(&self, vector: &Vector<N>) -> Vector<N> {
        let mapped = self.0.iter().zip(vector.0.iter()).map(|(a, b)| b - a);
    ...
    pub fn to_hashkey(&self) -> HashKey<N> {
        let bit_iter = self.0.iter().map(|x| x.to_bits());
    ...
    pub fn sq_euc_dis(&self, vector: &Vector<N>) -> f32 {
        ...
        // functional programming isn't a choice here
        // it's a lifestyle
    }
}
...
impl<const N: usize> ANNIndex<N> {
    fn build_hyperplane(
        ...
        let coefficients = all_vec[a].subtract_form(&all_vec[b]);
        let point_on_plane = all_vec[a].avg(&all_vec[b]);
        ...
        for &id in indexes.iter() {
            if hyperplane.point_is_above(&all_vec[id]) {
                above.push(id);
            } else {
                below.push(id);
            }
