...
def convert_raw_data_if_needed(data_dir: str, input_vec_path: str):
    ...
    processed_npz_path = os.path.join(
        data_dir, f"faiss_{input_file_no_ext}.npz")
    processed_pkl_path = os.path.join(
        data_dir, f"faiss_{input_file_no_ext}.pkl")
    if not os.path.exists(processed_npz_path) or not os.path.exists(processed_pkl_path):
        ...
        print(
            f"Loaded in {sample_data.shape}-shape input data in {end-start} seconds")
...
def index_and_check_runtime(
    ...
    print(
        f"ef_search={ef_search}, ef_construction={ef_construction}, max_node_size={max_node_size}, top_k={top_k}")
    ...
    search_data = data[np.random.choice(
        data.shape[0], size=1000)].reshape(1000, 1, -1)
    sch_start = time.time()
    for i in range(1000):
    ...
    print(
        f"Average Euclidean Distance = {sum(all_distances)/len(all_distances)}")
...
parser.add_argument("--input-vec", type=str,
                    help="The path to the input vec file")
...
index = index_and_check_runtime(
    sample_data, save_all_distances_folder=args.data_dir)

# Now, evaluate FAISS results qualitatively
other_display_args = [20, sample_data,
                      word_to_index_map, index_to_word_map, index]
